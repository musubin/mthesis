...
datacox4.3 <- data1 %>%
  ...
  mutate(strata1=as.numeric(Q8),strata2=as.numeric(NA),strata3=as.numeric(NA)) %>% 
  mutate(Set1=NA) %>% mutate(Set2=NA) %>%  mutate(Set3=NA) %>% #inner_join(cpi,by=c("PANEL"="chousa")) %>%
  bind_rows() %>% as.data.frame() %>% dplyr::arrange(ID,PANEL) %>% dplyr::group_by(ID) %>% 
  dplyr::filter(PANEL==max(PANEL))

datacox.3 <- bind_rows(datacox1.3,datacox2.3,datacox3.3,datacox4.3)%>% #Q511,Q565,Q673,Q700,Q493BM
  ...
  mutate(ikukyu=ifelse(Q880<=3,1,0)) %>% mutate(first=ifelse(Q57>=1,1,0)) %>% mutate(second=ifelse(Q57>=2,1,0)) %>%
  mutate(third=ifelse(Q57>=3,1,0)) %>%mutate(PANELf=PANEL-1) %>% #mutate(censor1=ifelse(strata1>=0,1,0)) %>% 
  dplyr::select(-Q57,-Q377,-Q142,-Q66,-Q297F,-Q299A,-Q299F,-Q61SR,-Q60SR,-Q213,-Q3,-Q493BH,-Q495BH,-Q880,-Q67A)%>% 
  bind_rows() %>% as.data.frame() %>% dplyr::arrange(ID,PANEL) %>% dplyr::group_by(ID) %>% 
  dplyr::filter(PANEL==max(PANEL))

datacox.3[is.na(datacox.3)]<-"."

write.csv(datacox.3,"svdata.csv")
